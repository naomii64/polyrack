class LayoutEditor{
    static layout=document.getElementById("layout-div");
    static list=document.getElementById("layout-component-list");
    static addButton=document.getElementById("layout-add-button");
    static addList=document.getElementById("layout-add-list");
    static compAttribs=document.getElementById("component-attributes");
    static extrasCheckbox=document.getElementById("extras-checkbox");

    static components=[];

    static init(){
        LE.renderer=new Renderer(Main.SCREEN_HEIGHT,Main.SCREEN_HEIGHT);
        LE.renderer.createPerspective(70);
        LE.layout.appendChild(LE.renderer.canvas);
        LE.renderer.makeDraggable();
        ...
        const gridWidth=document.getElementById("layout-grid-width");
        gridWidth.value=project.layout.width;
        gridWidth.addEventListener('change',e=>{
            project.layout.width=parseInt(gridWidth.value);
            LE.updateGrid();
        })


        const gridHeight=document.getElementById("layout-grid-height");
        gridHeight.value=project.layout.height;
        gridHeight.addEventListener('change',e=>{
            project.layout.height=parseInt(gridHeight.value);
            LE.updateGrid();
        })
    ...
    static registerComponentTypes(){
        LE.components=[];
        const registerComponent=function(name,init=function(){},savableKeys=[]){
            const comp={};
            comp.type=name;
            ...
            comp.draw=function(){}
            comp.savableKeys=savableKeys;
            comp.savableKeys.push('name');
            comp.savableKeys.push('position');
            comp.init=init;
            return comp;
        }
    ...
    static animate(){
        ...
        requestAnimationFrame(LE.animate);
    }
    ...
    static vectorInput(arr,object){
        const addInput=function(arrayIndex){
            ...
            input.onchange=function(){
                arr[arrayIndex]=parseFloat(this.value);
            }
        }
        const addX=function(){
